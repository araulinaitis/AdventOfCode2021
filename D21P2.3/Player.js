...
export default class Player {
  ...
  playTurnAsChild(turnNum, opponents) {
    if (this.wonAt || this.didLose) return;
    let newUniverses = 0; // how many parallel universes created by opponents
  ...
  playTurnAsOpponent(turnNum) {
    // this will prevent playing recursion (instead of just using playTurn on opponents)
    let newUniverses = 0;
    for (let [offset, count] of Object.entries(counts)) {
      offset = parseInt(offset);
      const newPos = ((this.pos + offset - 1) % 10) + 1;
      ...
      // only count universes where the opponent player will be able to play again
      newUniverses += !newChild.wonAt ? newChild.count : 0;
