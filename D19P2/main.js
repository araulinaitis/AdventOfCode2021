...
for (let [thisIdx, thisScanner] of scanners.entries()) {
  for (let otherIdx = thisIdx + 1; otherIdx < scanners.length; ++otherIdx) {
    const otherScanner = scanners[otherIdx];
    const thisManhattanDist = thisScanner.calcManhattanDistance(otherScanner);
    if (thisManhattanDist > maxManhattanDist) {
      maxManhattanDist = thisManhattanDist;
    }
  }
}
console.log(maxManhattanDist);